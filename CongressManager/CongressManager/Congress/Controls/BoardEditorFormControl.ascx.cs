...
namespace CongressManager.Congress.Controls
{
    public partial class BoardEditorFormControl : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            FileUploadPath = Server.MapPath("./MyFiles");
        ...
        private void DisplayDataForModify()
        {
            ...
            var note = (new CongressRepository()).GetNoteById(Convert.ToInt32(_Id));
        ...
        private void DisplayDataForReply()
        {
            ...
            string content = 
                $"<br /><br />On {note.PostDate}, '{note.Name}' wrote:<hr />{note.Content}";
            ...
            if (txtFileName.PostedFile != null)
            {
                    ...
                    && txtFileName.PostedFile.ContentLength > 0)
                {
                    ...
                    else // BoardWrite, BoardReply
                    {
                        ...
                        //업로드 처리 : SaveAs()
                        txtFileName.PostedFile.SaveAs(
                            Path.Combine(FileUploadPath, _FileName));
                    }
                }
            }//파일 업로드 처리 끝
        }
