...
{
    public partial class ScheduleByDomainViewControl : System.Web.UI.UserControl
    {
        ...
        private void DisplayData()
        {
            ...
            #region 제목과 내용 출력
            using (IDataReader objDr = (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteReader("ViewScheduleByDomain", Session["UserID"].ToString(), Request["Num"]))
            {
                if (objDr.Read())
                {
        ...
        {
            (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteNonQuery("ModifyScheduleByDomain", Session["UserID"].ToString(), this.lstYear.SelectedValue, this.ddlMonth.SelectedValue, this.comDay.SelectedValue, this.ctlHour.SelectedValue, txtTitle.Text, txtContent.Text, Request["Num"]);
            btnList_Click(null, null);
        }
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteNonQuery("DeleteScheduleByDomain", Session["UserID"].ToString(), Request["Num"]);
            btnList_Click(null, null);
        }
        protected void btnList_Click(object sender, EventArgs e)
        {
            string strUrl = String.Format("ScheduleByDomainList.aspx?SYear={0}&SMonth={1}", strSYear, strSMonth);
