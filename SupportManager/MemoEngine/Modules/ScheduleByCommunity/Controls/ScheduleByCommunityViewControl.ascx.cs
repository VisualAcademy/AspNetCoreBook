...
{
    public partial class ScheduleByCommunityViewControl : System.Web.UI.UserControl
    {
        ...
        private void DisplayData()
        {
            ...
            #region 제목과 내용 출력
            using (IDataReader objDr = (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteReader("ViewScheduleByCommunity", Session["Community"].ToString(), Request["Num"]))
            {
                if (objDr.Read())
                {
        ...
        {
            (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteNonQuery("ModifyScheduleByCommunity", Session["Community"].ToString(), this.lstYear.SelectedValue, this.ddlMonth.SelectedValue, this.comDay.SelectedValue, this.ctlHour.SelectedValue, txtTitle.Text, txtContent.Text, Request["Num"]);
            btnList_Click(null, null);
        }
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            (new DatabaseProviderFactory()).Create("ConnectionString").ExecuteNonQuery("DeleteScheduleByCommunity", Session["Community"].ToString(), Request["Num"]);
            btnList_Click(null, null);
        }
        protected void btnList_Click(object sender, EventArgs e)
        {
            string strUrl = String.Format("ScheduleByCommunityList.aspx?SYear={0}&SMonth={1}", strSYear, strSMonth);
