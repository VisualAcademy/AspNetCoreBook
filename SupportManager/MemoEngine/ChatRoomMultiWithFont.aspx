<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ChatRoomMultiWithFont.aspx.cs" Inherits="MemoEngine.ChatRoomMultiWithFont" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>채팅방</title>

    <link type="text/css" rel="stylesheet" href="Css/ChatStyle.css" />
    <link rel="stylesheet" href="/Css/JQueryUI/themes/base/jquery.ui.all.css" />

    <!--Script references. -->

    <!--Reference the jQuery library. -->
    <script src="Scripts/jquery-2.1.3.min.js"></script>
    <script src="/Scripts/ui/jquery.ui.core.js"></script>
    <script src="/Scripts/ui/jquery.ui.widget.js"></script>
    <script src="/Scripts/ui/jquery.ui.mouse.js"></script>
    <script src="/Scripts/ui/jquery.ui.draggable.js"></script>
    <script src="/Scripts/ui/jquery.ui.resizable.js"></script>


    <!--Reference the SignalR library. -->
    <script src="Scripts/jquery.signalR-2.2.0.min.js"></script>

    <!--Reference the autogenerated SignalR hub script. -->
    <script src="/signalr/hubs"></script>

    <script type="text/javascript">

        $(function () {
            //[0] 화면 설정
            setScreen(false);

            //[1] 서버측에 설정된 ChatHub 클래스를 가리키는 프록시 개체 생성: ChatHub 클래스이지만, chat로 참조(HubName 특성에 의해서) 
            var chat = $.connection.chatMultiAddFont;

            //[2] 클라이언트측 코드 구현
            registerClientMethods(chat);

            //[3] 허브 시작 및 메시지 전송 등과 관련된 코드 구현
            $.connection.hub.start().done(function () {
                registerEvents(chat);
            });
        });


        // 로그인 화면과 채팅 화면 토글
        function setScreen(isLogin) {

            if (!isLogin) {

                $("#divChat").hide();
                $("#divLogin").show();
            }
            else {

                $("#divChat").show();
                $("#divLogin").hide();
            }

        }

        function registerEvents(chat) {

            $("#btnStartChat").click(function () {

                var name = $("#txtNickName").val();
                if (name.length > 0) {
                    chat.server.connect('<%= Request["BoardName"] %>', name);
                }
                else {
                    alert("Please enter name");
                }

            });


            $('#btnSendMsg').click(function () {


                var fSize = $("#optSize").val(); // 폰트 사이즈
                var fColor = $("#optColor").val(); // 폰트 크기 


                var msg = $("#txtMessage").val();
                if (msg.length > 0) {

                    var userName = $('#hdUserName').val();
                    //chat.server.sendMessageToAll(userName, msg);
                    chat.server.sendMessageToAll('<%= Request["BoardName"] %>', userName, msg, fSize, fColor);
                    $("#txtMessage").val('');
                }
            });


            $("#txtNickName").keypress(function (e) {
                if (e.which == 13) {
                    $("#btnStartChat").click();
                }
            });

            $("#txtMessage").keypress(function (e) {
                if (e.which == 13) {
                    $('#btnSendMsg').click();
                }
            });


        }

        function registerClientMethods(chat) {

            // Calls when user successfully logged in
            chat.client.onConnected = function (id, userName, allUsers, messages) {

                setScreen(true);

                $('#hdId').val(id);
                $('#hdUserName').val(userName);
                $('#spanUser').html(userName);

                // Add All Users
                for (i = 0; i < allUsers.length; i++) {
                    if (allUsers[i].RoomName == '<%= Request["BoardName"] %>') {
                        AddUser(chat, allUsers[i].ConnectionId, allUsers[i].UserName);
                    }
                }

                // Add Existing Messages
                for (i = 0; i < messages.length; i++) {
                    if (messages[i].RoomName == '<%= Request["BoardName"] %>') {
                        AddMessage(messages[i].UserName, messages[i].Message);
                    }
                }
            }


            // 새로운 사용자가 채팅방에 접속했을 때 사용자 리스트에 추가하기
            chat.client.onNewUserConnected = function (room, id, name) {
                if (room == '<%= Request["BoardName"] %>') {
                    AddUser(chat, id, name);
                }
            }


            // On User Disconnected
            chat.client.onUserDisconnected = function (id, userName) {

                $('#' + id).remove();

                var ctrId = 'private_' + id;
                $('#' + ctrId).remove();


                var disc = $('<div class="disconnect">"' + userName + '" logged off.</div>');

                $(disc).hide();
                $('#divusers').prepend(disc);
                $(disc).fadeIn(200).delay(2000).fadeOut(200);

            }

            chat.client.messageReceived = function (userName, message, fSize, fColor) {

                AddMessage(userName, message, fSize, fColor);
            }


            chat.client.sendPrivateMessage = function (windowId, fromUserName, message) {

                var ctrId = 'private_' + windowId;


                if ($('#' + ctrId).length == 0) {

                    createPrivateChatWindow(chat, windowId, ctrId, fromUserName);

                }

                $('#' + ctrId).find('#divMessage').append('<div class="message"><span class="userName">' + fromUserName + '</span>: ' + message + '</div>');

                // set scrollbar
                var height = $('#' + ctrId).find('#divMessage')[0].scrollHeight;
                $('#' + ctrId).find('#divMessage').scrollTop(height);

            }

        }

        function AddUser(chat, id, name) {

            var userId = $('#hdId').val();

            var code = "";

            if (userId == id) {

                code = $('<div class="loginUser">' + name + "</div>");

            }
            else {

                code = $('<a id="' + id + '" class="user" >' + name + '<a>');

                $(code).dblclick(function () {

                    var id = $(this).attr('id');

                    if (userId != id) {
                        OpenPrivateChatWindow(chat, id, name); // 1대1 채팅창 열기
                    }

                });
            }

            $("#divusers").append(code);

        }

        function AddMessage(userName, message, fSize, fColor) {
            $('#divChatWindow').append('<div class="message"><span class="userName">' + userName + '</span>: <span  style="color:' + fColor + ';font-size:' + fSize + 'pt;">' + message + '</span></div>');

            var height = $('#divChatWindow')[0].scrollHeight;
            $('#divChatWindow').scrollTop(height);
        }

        function OpenPrivateChatWindow(chat, id, userName) {

            var ctrId = 'private_' + id;

            if ($('#' + ctrId).length > 0) return;

            createPrivateChatWindow(chat, id, ctrId, userName);

        }

        function createPrivateChatWindow(chat, userId, ctrId, userName) {

            var div = '<div id="' + ctrId + '" class="ui-widget-content draggable" rel="0">' +
                       '<div class="header">' +
                          '<div  style="float:right;">' +
                              '<img id="imgDelete"  style="cursor:pointer;" src="/Images/delete.png"/>' +
                           '</div>' +

                           '<span class="selText" rel="0">' + userName + '</span>' +
                       '</div>' +
                       '<div id="divMessage" class="messageArea">' +

                       '</div>' +
                       '<div class="buttonBar">' +
                          '<input id="txtPrivateMessage" class="msgText" type="text"   />' +
                          '<input id="btnSendMessage" class="submitButton button" type="button" value="Send"   />' +
                       '</div>' +
                    '</div>';

            var $div = $(div);

            // DELETE BUTTON IMAGE
            $div.find('#imgDelete').click(function () {
                $('#' + ctrId).remove();
            });

            // Send Button event
            $div.find("#btnSendMessage").click(function () {

                $textBox = $div.find("#txtPrivateMessage");
                var msg = $textBox.val();
                if (msg.length > 0) {

                    chat.server.sendPrivateMessage(userId, msg);
                    $textBox.val('');
                }
            });

            // Text Box event
            $div.find("#txtPrivateMessage").keypress(function (e) {
                if (e.which == 13) {
                    $div.find("#btnSendMessage").click();
                }
            });

            AddDivToContainer($div);

        }

        function AddDivToContainer($div) {
            $('#divContainer').prepend($div);

            $div.draggable({

                handle: ".header",
                stop: function () {

                }
            });

            ////$div.resizable({
            ////    stop: function () {

            ////    }
            ////});

        }

    </script>
</head>
<body>
    <form id="form1" runat="server">
        <div id="header">
            대화방(Chat Room)
        </div>
        <br />
        <br />
        <br />

        <div id="divContainer">
            <div id="divLogin" class="login">
                <div>
                    대화명(Name):
                    <br />
                    <input id="txtNickName" type="text" class="textBox" />
                </div>
                <div id="divButton">
                    <input id="btnStartChat" type="button" class="submitButton" value="참여하기" />
                </div>
            </div>

            <div id="divChat" class="chatRoom">
                <div class="title">
                    대화방에 오신걸 환영합니다. [<span id='spanUser'></span>]

                </div>
                <div class="content">
                    <div id="divChatWindow" class="chatWindow">
                    </div>
                    <div id="divusers" class="users">
                    </div>
                </div>
                <div class="messageBar">
                    <input class="textbox" type="text" id="txtMessage" />
                    <input id="btnSendMsg" type="button" value="Send" class="submitButton" />
                </div>
                <div class="fontBar">
                    <select id="optSize">
                        <option value="11">11</option>
                        <option value="12">12</option>
                        <option value="13">13</option>
                        <option value="14">14</option>
                        <option value="20">20</option>
                    </select>
                    <select id="optColor">
                        <option value="black">black</option>
                        <option value="red">red</option>
                        <option value="green">green</option>
                    </select>
                </div>
            </div>

            <input id="hdId" type="hidden" />
            <input id="hdUserName" type="hidden" />
        </div>

    </form>
</body>
</html>
