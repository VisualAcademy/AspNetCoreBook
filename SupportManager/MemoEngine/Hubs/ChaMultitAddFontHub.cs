...
{
    [HubName("chatMultiAddFont")]
    public class ChatMultiAddFontHub : Hub
    {
        /// <summary>
        ...
        public void SendMessageToAll(string room, string userName, string message, string fSize, string fColor)
        {
            // 최근 200개의 메시지를 캐시에 저장
            AddMessageinCache(userName, message);

            // 채팅 메시지를 브로드캐스팅
            Clients.Group(room).messageReceived(userName, message, fSize, fColor);
        }
