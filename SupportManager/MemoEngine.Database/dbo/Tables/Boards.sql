--[!] 게시판 관리용 테이블: 다중으로 생성되는 모든 게시판 관련 정보는 Boards 테이블에서 관리됨
CREATE TABLE [dbo].[Boards]
(
    [TID]           [Int] IDENTITY(1,1) NOT NULL Primary Key,	--일련번호(BoardId, Id)
    ...
    --
    [SysopUID]      [Int] NULL DEFAULT ((0)),					--회원제 연동시 시삽 권한 부여(SysopUserId, SysopUsername)
    [IsPublic]      [Bit] NULL DEFAULT ((1)),					--익명사용자(true_1) / 회원 전용(false_0)게시판 구분
    ...
    [MainShowList]	[Bit] Default(1),								-- 포탈(메인 요약 게시판)에 표시할지 안할지 결정 : 0 : 숨김, 1 :  표시
    [BoardStyle]	[Int] NULL DEFAULT ((0)),					-- 게시판 스타일 : 기본(0), 강좌(1), 메모장(2), 이벤트(3), ... 

    -- 게시판 상단과 하단에 HTML 인클루드 용도
    [HeaderHtml]	[NVarChar](4000) NULL,						-- 게시판 상단 HTML 인클루드
    [FooterHtml]	[NVarChar](4000) NULL						-- 게시판 하단 HTML 인클루드
)
Go
